...
class Import extends AbstractJob
{
    ...
    public function perform()
    {
        ...
        foreach ($zParentItems as $zParentItemKey => &$zParentItem) {
            ...
            //update item instead of batch create
            $title = $oItem["dcterms:title"][0]["@value"];    

            //add resource template to oItem
            $oItem["o:resource_template"] = ["o:id" => 2];

            //$this->logger->info(print_r($title, true));
            parse_str("property[0][joiner]=and&property[0][property]=1&property[0][type]=eq&property[0][text]={$title}",$query);
            $result = $api->search("items",$query)->getContent();
            if (isset($result[0])){                
                $result = $result[0];
                // Update the item.
                $api->update('items', $result->id(), $oItem, [], ['isPartial' => true, 'flushEntityManager' => true]);
                $this->logger->info("updated ".$result->id());
                $result = "";
            }else{
